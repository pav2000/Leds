...
public: 
  ...
  uint8_t get_code_at(uint16_t index);
  cRGB get_crgb_at(uint16_t index);
  
  uint8_t set_code_at(uint16_t index, uint8_t px_value);
...
private:
  uint16_t count_led;  // число светодиодов
  uint8_t *pixels;     // маска грящийх светодиодов
